...
function Column({ id, todos, index }: Props) {
  const { searchString, setNewTodoType } = useBoardStore();
  const { openModal } = useAddTodoModalStore();
          ...
          <Droppable droppableId={index.toString()} type="card">
            {(provided, snapshot) => (
              <div
                ...
                className={`p-2 rounded-2xl shadow-sm ${snapshot.isDraggingOver ? 'bg-purple-200' : 'bg-white/50'}`}
              >
                <h2 className="flex justify-between p-2 text-xl font-bold">
                  {idToColumnText[id]}
                  <span className="px-2 py-1 text-sm font-normal bg-gray-200 rounded-full test-gray-500">
                    {!searchString
                      ? todos.length
